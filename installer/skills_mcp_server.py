...
# -*- coding: utf-8 -*-
"""
Consolidated Skills MCP Server v3.0.0
======================================
A zero-dependency Python MCP server (JSON-RPC over stdio) that:
  1. Scans ALL configured skills directories for available skills
  ...
  5. Works on both Windows (PowerShell) and WSL (Ubuntu)
...
# The SINGLE consolidated skills directory (source of truth, Git-synced)
# All skills live here. No other directories are scanned.
...
# Single directory to scan — everything is consolidated here
SKILL_DIRS_WIN = [
...
class SkillRegistry:
    ...
    def _parse_frontmatter(self, skill_md_path):
        ...
        try:
            with open(skill_md_path, "r", encoding="utf-8-sig", errors="ignore") as f:
    ...
    def _scan(self):
        ...
        for win_dir, label, priority in SKILL_DIRS_WIN:
            resolved = resolve_path(win_dir)
            log(f"Scanning [{label}] (priority={priority}): {resolved}")
            skill_count = self._scan_directory(resolved, label, priority)
            log(f"  -> {skill_count} skills from {label}")
    ...
    def get_instructions(self, skill_name):
        ...
        try:
            with open(info["skill_md_path"], "r", encoding="utf-8-sig", errors="ignore") as f:
                return {
                    "name": info["name"],
                    "description": info["description"],
                    "source_dir": info["source_dir"],
    ...
    def consolidate(self, extra_scan_dirs=None):
        """
        Scan scattered directories for skills and COPY unique ones
        ...
        scan_dirs = []
        for win_dir, label, priority in SKILL_DIRS_WIN:
            if "repo:" not in label:  # Don't consolidate FROM the repo
                scan_dirs.append(resolve_path(win_dir))
        if extra_scan_dirs:
...
TOOL_DEFINITIONS = [
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "source_filter": {
                    "type": "string",
                    "description": "Optional filter: 'repo' for GitHub-synced skills, 'local' for IDE-installed skills, 'antigravity' for antigravity skills. Omit to list all.",
                }
            },
            "required": [],
        },
    ...
    },
    {
        ...
        "description": "Scan scattered skill directories (IDE-installed, plugins, antigravity) for unique skills and COPY them into the centralized Git repository. Deduplicates by folder name. Use to organize all skills into a single source of truth.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "extra_dirs": {
...
# ---------------------------------------------------------------------------
# JSON-RPC Transport (with BOM handling)
# ---------------------------------------------------------------------------
...
def main():
    ...
    log("=" * 60)
    log(f"Consolidated Skills MCP Server v{SERVER_INFO['version']}")
    log(f"Running in WSL: {IS_WSL}")
    log(f"Platform: {'WSL/Linux' if IS_WSL else 'Windows'}")
    ...
    for line in sys.stdin:
        ...
        # Also handle raw BOM bytes
        if isinstance(line, str) and line.startswith("\xef\xbb\xbf"):
            line = line[3:]

        try:
            request = json.loads(line)
        except json.JSONDecodeError as e:
            log(f"Invalid JSON: {e} | raw={repr(line[:200])}")
